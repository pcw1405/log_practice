...

{
  ...
  grep "Failed login" "$LOG_FILE" | extract_ip | sort | uniq -c | sort -nr | head -n 5 || true

  echo ""
  echo "[차단 후보(IP) - ${ALERT_THRESHOLD}회 이상]"
  grep "Failed login" "$LOG_FILE" | extract_ip | sort | uniq -c \
    | awk -v th="$ALERT_THRESHOLD" '$1>=th {print}' | sort -nr || true

  echo ""
  echo "[공격 타겟 URI TOP5]"
  grep "Failed login" "$LOG_FILE" | extract_uri | sort | uniq -c | sort -nr | head -n 5 || true

  echo ""
  echo "[타겟 계정 USER TOP5]"
  grep "Failed login" "$LOG_FILE" | extract_user | sort | uniq -c | sort -nr | head -n 5 || true

  echo ""
  echo "[최근 Failed login 10줄]"
  grep "Failed login" "$LOG_FILE" | tail -n 10 || true
